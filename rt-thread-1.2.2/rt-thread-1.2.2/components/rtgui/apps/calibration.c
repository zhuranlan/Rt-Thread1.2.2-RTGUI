...
//static struct calibration_session *calibration_ptr __attribute__((at(0X10000000+4))) = {0};
static struct calibration_session *calibration_ptr={0};
/*注意！！！！！！！！！！这个部分对于校验非常的重要  要根据屏幕的实际情况去设置*/
...
//		rt_kprintf("calibration_ptr->data.min_x:%d \n",calibration_ptr->data.min_x);
//		rt_kprintf("calibration_ptr->data.max_x:%d \n",calibration_ptr->data.max_x);
//		rt_kprintf("calibration_ptr->data.min_y:%d \n",calibration_ptr->data.min_y);
//		rt_kprintf("calibration_ptr->data.max_y:%d \n",calibration_ptr->data.max_y);
...
static rt_bool_t calibration_event_handler(struct rtgui_object *object, struct rtgui_event *event)
{
    ...
    rtgui_rect_t label_rect = {50,100,480,320};
    switch (event->type)
    {
...
void calibration_init(void)
{
    ...
    tid = rt_thread_create("cali", calibration_entry, RT_NULL, 1024*2, 20, 20);
    if (tid != RT_NULL)
