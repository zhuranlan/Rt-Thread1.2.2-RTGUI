...
//		rt_kprintf("%d \n" ,rtgui_rect_width(view->view_rect)*view->page_count);
//		rt_kprintf("%d \n" ,rtgui_rect_height(view->view_rect));
//		if(view->view_buffer==RT_NULL)
...
static void app_mainmenu_init(void)
{
    ...
    if (win != RT_NULL)
    {
        ...
        bg_image = rtgui_image_create(image_path, RT_TRUE);
        if (bg_image == RT_NULL)
        {
            rt_kprintf("open image:%s failed!\n", image_path);
        }
        ycircle_image = rtgui_image_create("/SD/ycircle.png", RT_TRUE);
        gcircle_image = rtgui_image_create("/SD/gcircle.png", RT_TRUE);
