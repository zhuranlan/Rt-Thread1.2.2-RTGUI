...
void tcpserv(unsigned int port)
{
          ...
          // send(connected, send_data, strlen(send_data), 0);
           ...
           else
           {
             ...
             rt_kprintf("%s" , recv_data);
						// if(strcmp(recv_data,"电压")==0)
						 send(connected, store_data, sizeof(store_data), 0);
						 //send(connected, recv_data, sizeof(recv_data), 0);
...
static rt_uint8_t TCPserver_stack[ 1024 ];
static struct rt_thread TCPSERVER;
void TCP_Server_init(void)
{
	rt_err_t result;
	
		result = rt_thread_init(&TCPSERVER,
		                    "TCP_Server",
		                    TCP_Server_Thread, 
							RT_NULL,
		                    &TCPserver_stack[0],
							sizeof(TCPserver_stack),
							1,
							5
							);
