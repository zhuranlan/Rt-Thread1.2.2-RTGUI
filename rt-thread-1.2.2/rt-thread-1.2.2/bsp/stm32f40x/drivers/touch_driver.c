...
TOUCH INT: PC1
*/
#define IS_TOUCH_UP()     GPIO_ReadInputDataBit(GPIOC, GPIO_Pin_1)
...
/*根据需要去进行触摸的翻转*/
#define Touch_X_Turn  1
#define Touch_Y_Turn  0
/*
7  6 - 4  3      2     1-0
...
#if USE_HORIZONTAL==1
#define TOUCH_MSR_Y     (START | MEASURE_X | MODE_12BIT | DIFFERENTIAL | POWER_MODE0)
#define TOUCH_MSR_X     (START | MEASURE_Y | MODE_12BIT | DIFFERENTIAL | POWER_MODE0)
#else
...
#else
#define MIN_X_DEFAULT   150
#define MAX_X_DEFAULT   1880
#define MIN_Y_DEFAULT   188
#define MAX_Y_DEFAULT   1853
#endif
...
static rt_uint8_t touch_stack[ 512 ] __attribute__((at(0X10000000)));
static struct rt_thread touch_driver;
rt_err_t rtgui_touch_hw_init(const char * spi_device_name)
{
	...
	result = rt_thread_init(&touch_driver,
		                    "touch",
		                    touch_thread_entry, 
							RT_NULL,
		                    &touch_stack[0],
							sizeof(touch_stack),
							RTGUI_SVR_THREAD_PRIORITY-1,
							5
							);

	if (result == RT_EOK)
	{
        rt_thread_startup(&touch_driver);
	}
