...
//rt_uint8_t tftp_buffer[1024*8 + 4] __attribute__((at(0X10000000+1024*10+8)))= {0};
rt_uint8_t tftp_buffer[1024*2];
/* tftp client */
...
void tftp_put(char* host,char* dir,char* filename)
{
	int fd, sock_fd, sock_opt;
	struct sockaddr_in tftp_addr, from_addr;
	...
	/* make local file name tftp_put("192.168.1.105","/SD/DATA","??A_1.DAT");*/
	rt_snprintf((char*)tftp_buffer, sizeof(tftp_buffer),
		"%s/%s", dir, filename);
